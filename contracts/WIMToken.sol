...
/// @title WIMToken - Crowdfunding code for the WeInMeIO Project
/// @author Jan Jansen Van Vuuren, Neil Craig
contract WIMToken is StandardToken, Ownable {

    using SafeMath for uint;
    string public constant name = "WeInMe";
    string public constant symbol = "WIM";
    ...
    address public WIMTokenFactory;
    ...
    //@notice  Constructor of WIMToken
    //@param `_WIMTokenFactory` - multisigwallet address to store proceeds.
    //@param `_icedWallet` - Multisigwallet address to which unsold tokens are assigned
    function WIMToken(address _WIMTokenFactory, address _icedWallet) {
    ...
    function finalize() isFundable onlyOwner external {
      ...
      //transfer any balance available to WeInMe Multisig Wallet
    ...
    function refund() isFundable external {
      ...
      uint wimValue = balances[msg.sender];
      if(wimValue == 0) throw;

      balances[msg.sender] = 0;

      uint ethValue = wimValue.mul(tokenPrice).div(1e18);
